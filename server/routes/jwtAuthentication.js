...
router.post("/register", async (req, res) => {
  try {
    ...
    const newUser = await pool.query(
      "INSERT INTO account (user_name, displayed_name, user_password) VALUES ($1, $2, $3) RETURNING * ",
      [username, username, bcryptedPassword]
    );
...
router.post("/login", async (req, res) => {
  try {
    ...
    const isCorrectPassword = await bcrypt.compare(password, bcryptedPassword);
